...
\begin{frame}{Installing ROS 2}{Sourcing the installation}
  ...
  \newline\newline
  so that your shell, and all its child processes from then on, will know the \textbg{paths} of all the executables and shared objects (libraries) installed by ROS 2.
  \newline\newline
  Additional commands are required to set up \textbg{command line completion} and \textbg{environment variables}.\\
...
\begin{frame}{Language support}{Low- vs high-level programming}
  ...
  \end{itemize}
  You can develop software packages using \textbg{only one} of them, or \textbg{both} at the same time.
\end{frame}
\begin{frame}{Language support}{Low- vs high-level programming}
...
\begin{frame}{Language support}{The build system}
  ...
  \end{itemize}
  All its configuration for each package can be found in a \texttt{package.xml} \textbg{manifest file}.
\end{frame}
\begin{frame}{Language support}{CMake}
  \begin{columns}
    \column{.5\textwidth}
    CMake is a \textbg{cross-platform} build configuration generator, which allows to build software using a \textbg{single, unified syntax} on all supported platforms.\\
...
\begin{frame}{Language support}{setuptools}
  \begin{columns}
    ...
    \begin{itemize}
      ...
      \item \textbg{\texttt{setup.cfg}} files, which specifies the location of all \textbg{executable scripts} in the package.
    \end{itemize}
...
\begin{frame}{Language support}{Building packages with colcon}
  ...
  \begin{itemize}
    ...
    \item \texttt{-{}-symlink-install} to \textbg{symlink} the built packages into the \texttt{install/} directory, instead of copying them;
...
\begin{frame}{The workspace}{Anatomy of a ROS 2 development directory}
  ...
  \begin{itemize}
    ...
    \item \texttt{install/} (autogenerated), which contains the \textbg{built packages};
